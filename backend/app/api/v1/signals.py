...
async def ingest_signal(
    ...
    device_state = await redis.get(f"device:{heartbeat.device_id}:state")
    if device_state == "blocked":
    ...
    background_tasks.add_task(
        analyze_signal.delay, signal_id=signal_id, payload=heartbeat.payload.dict(), device_id=heartbeat.device_id
    )
